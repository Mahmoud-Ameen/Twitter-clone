...
export default function () {
  ...
  dom.querySelector('form').onsubmit = () => {
    const username = dom.querySelector('input.username').value
    const password = dom.querySelector('input.password').value
    try {
      fakeAuthService.logIn(username, password)
      goToPage('home')
    } catch (error) {
      console.error(error)
    }
