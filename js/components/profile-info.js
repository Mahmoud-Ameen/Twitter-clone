...
export default function ProfileInfo (user) {
  ...
  const currentUserId = fakeAuthService.getCurrentUser()
					...
					<div class="body">
						<div class="upper">
							...
							<div class="actions">
								...
								<button class="toggleFollow btn btn--outline action"> 
									${followers.has(currentUserId) ? 'Unfollow' : 'Follow'}
								</button>
							</div>
						</div>
						<p class="profile__name text-heading mt-2">
							${name}
							<!--  render the verified icon if verified -->
							${
                isVerified
                  ? `<span class='verified color-primary'>
                  ...
                  : ''
						...
						<span class="profile__username text-secondary mb-3">@${username}</span>
						...
						<div class="my-2">
							<span class="following text-secondary">
								<span class="text-bold">${followings.size}</span>
							<a href=${'#followings?' + username}>	Following</a>
							</span>
							<span class="followers ml-2 text-secondary">
								<span class="text-bold">${followers.size}</span> 
  ...
  dom.querySelector('.toggleFollow').addEventListener('click', () => {
    fakeUsersService.handleFollowUser(user.username)
  })
