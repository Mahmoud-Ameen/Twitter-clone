...
export default function navbar (active) {
  ...
  const currentUserData = fakeUsersService.getUserData(
  ...
  dom.innerHTML = `
    ...
    <ul>
      ...
      <li class='nav__link profile'>
        <a href=${'#user?' + currentUserData.username}>
          <span class='icon nav__link__icon'>
            <svg
              viewBox='0 0 24 24'
              aria-hidden='true'
      ...
      <li class='nav__link settings'>
        <span class='icon nav__link__icon'>
          <svg
            viewBox='0 0 24 24'
            aria-hidden='true'
            class='r-14j79pv r-4qtqp9 r-yyyyoo r-1q142lx r-1xvli5t r-1b7u577 r-dnmrzs r-bnwqim r-1plcrui r-lrvibr'
          >
            <g>
          ...
          <p class='nav__link__title'>Settings</p>
        </span>
      </li>
      <li class='tweet-btn'>
      ...
      <div class="currentUserCard">
        <img src=${currentUserData.image}  width="40px" height="40px" alt=${
    currentUserData.username
  }>
        <div class="user__info">
          <p class="name text-bold user__name">${currentUserData.name}</p>
          <p class="username text-secondary user__username">@${
            currentUserData.username
          }</p>
        </div>
      </div>
      </div>
      <div class="settingsModalContainer "></div>
		`
  ...
  const openSettingsModal = () => {
    ...
    dom
      .querySelector('.settingsModalContainer')
      .appendChild(
        settingsModal(
          () => (dom.querySelector('.settingsModalContainer').innerHTML = '')
        )
